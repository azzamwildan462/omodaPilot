...
namespace ublox_node
{
  ...
  void UbloxFirmware6::subscribe(std::shared_ptr<ublox_gps::Gps> gps)
  {
    ...
    gps->subscribe<ublox_msgs::msg::NavPOSLLH>(std::bind(
                                                   &UbloxFirmware6::callbackNavPosLlh, this, std::placeholders::_1),
                                               1);
    gps->subscribe<ublox_msgs::msg::NavSOL>(std::bind(
                                                // Subscribe to Nav SOL
                                                &UbloxFirmware6::callbackNavSol, this, std::placeholders::_1),
                                            1);
    // Subscribe to Nav VELNED
    gps->subscribe<ublox_msgs::msg::NavVELNED>(std::bind(
                                                   &UbloxFirmware6::callbackNavVelNed, this, std::placeholders::_1),
                                               1);

    // Subscribe to Nav SVINFO
    if (getRosBoolean(node_, "publish.nav.svinfo"))
    {
      gps->subscribe<ublox_msgs::msg::NavSVINFO>([this](const ublox_msgs::msg::NavSVINFO &m)
                                                 { nav_svinfo_pub_->publish(m); },
    ...
    if (getRosBoolean(node_, "publish.mon.hw"))
    {
      gps->subscribe<ublox_msgs::msg::MonHW6>([this](const ublox_msgs::msg::MonHW6 &m)
                                              { mon_hw_pub_->publish(m); },
                                              1);
    }
  }
