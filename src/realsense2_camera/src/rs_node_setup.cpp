...
void BaseRealSenseNode::setAvailableSensors()
{
    ...
    if (std::string(serial_no).substr(0, 6) == FRAMOS_SN_START) {
        auto ip_address = _dev.get_info(RS2_CAMERA_INFO_IP_ADDRESS);
        ROS_INFO_STREAM("Device IP Address: " << ip_address);
    }
    ...
    for(auto&& sensor : _dev_sensors) {
        ...
        if (sensor.is<rs2::depth_sensor>() || sensor.is<rs2::color_sensor>()) {
            ...
            rosSensor = std::make_unique<RosSensor>(
                sensor, 
                _parameters, 
                frame_callback_function, 
                update_sensor_func, 
                hardware_reset_func, 
                _diagnostics_updater, 
                _logger, 
                _use_intra_process, 
                _dev.is<playback>()
            );
...
{
    _asyncer.start(
        [this](rs2::frame f) { frame_callback(f); }
    );
}



void BaseRealSenseNode::startPublishers
(
    const std::vector<stream_profile>& profiles, 
    const RosSensor& sensor
)
{
    ...
    for (auto& profile : profiles) {
        ...
        if (profile.is<rs2::video_stream_profile>()) {
            if(profile.stream_type() == RS2_STREAM_COLOR) { _is_color_enabled = true; }
            else if (profile.stream_type() == RS2_STREAM_DEPTH) { _is_depth_enabled = true; }
            ...
            _info_publishers[sip] = _node.create_publisher<sensor_msgs::msg::CameraInfo>(
                camera_info.str(),
            ...
            if (_align_depth_filter->is_enabled() && (sip != DEPTH) && sip.second < 2) {
                ...
                _depth_aligned_info_publisher[sip] = _node.create_publisher<sensor_msgs::msg::CameraInfo>(
            ...
            if(profile.stream_type() == RS2_STREAM_ACCEL) { _is_accel_enabled = true; }
            else if (profile.stream_type() == RS2_STREAM_GYRO) { _is_gyro_enabled = true; }

            std::stringstream data_topic_name, info_topic_name;
            data_topic_name << "~/" << stream_name << "/sample";

            _imu_publishers[sip] = _node.create_publisher<sensor_msgs::msg::Imu>(
                data_topic_name.str(),
            ...
            _imu_info_publishers[sip] = _node.create_publisher<IMUInfo>(
                info_topic_name.str(),
                rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(info_qos), info_qos)
            );
        ...
        _metadata_publishers[sip] = _node.create_publisher<realsense2_camera_msgs::msg::Metadata>(
            topic_metadata, 
            rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(info_qos), info_qos)
        );
        ...
        if (!((rs2::stream_profile)profile==(rs2::stream_profile)_base_profile)) {
            ...
            _extrinsics_publishers[sip] = _node.create_publisher<realsense2_camera_msgs::msg::Extrinsics>(
                topic_extrinsics,
    ...
    if (_is_accel_enabled && _is_gyro_enabled && (_imu_sync_method > imu_sync_method::NONE)) {
        ...
        _synced_imu_publisher = std::make_shared<SyncedImuPublisher>(
            _node.create_publisher<sensor_msgs::msg::Imu>("~/imu", rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(qos), qos))
        );
    }
}
...
void BaseRealSenseNode::startRGBDPublisherIfNeeded()
{
    ...
    if(_enable_rgbd && !_rgbd_publisher) {
        if (_sync_frames && _is_color_enabled && _is_depth_enabled && _align_depth_filter->is_enabled()) {
            ...
            // adding "~/" to the topic name will add node namespace and node name to the topic
            // see "Private Namespace Substitution Character" section on https://design.ros2.org/articles/topic_and_service_names.html
            _rgbd_publisher = _node.create_publisher<realsense2_camera_msgs::msg::RGBD>(
                "~/rgbd",
                rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(qos), qos)
            );
        }
        else {
            ROS_ERROR("In order to get rgbd topic enabled, you should enable: color stream, depth stream, sync_mode and align_depth");
        }
    }
}
...
void BaseRealSenseNode::stopPublishers
(
    const std::vector<stream_profile>& profiles
)
{
...
void BaseRealSenseNode::publishServices()
{
    ...
    _device_info_srv = _node.create_service<realsense2_camera_msgs::srv::DeviceInfo>(
            "~/device_info",
            [&](const realsense2_camera_msgs::srv::DeviceInfo::Request::SharedPtr req, realsense2_camera_msgs::srv::DeviceInfo::Response::SharedPtr res) {
