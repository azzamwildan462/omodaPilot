...
)
{
    ...
    return ((libname_to_rosname.find(rs2_name) == libname_to_rosname.end()) ? 
                rs2_name : 
                name_iter->second
    );
}
...
std::string ros_stream_to_string
(
    rs2_stream stream
)
{
...
std::string create_graph_resource_name
(
...
)
{
    ...
    std::replace_if(
        fixed_name.begin(), 
        fixed_name.end(), 
        [](const char c) { return !(realsense2_camera::isValidCharInName(c)); },
        '_'
    );
...
rs2_format string_to_rs2_format
(
    std::string str
)
{
...
const rmw_qos_profile_t qos_string_to_qos
(
    std::string str
)
{
