...
(
    ...
    _min_freq(expected_frequency), 
    _max_freq(expected_frequency),
...
FrequencyDiagnostics::FrequencyDiagnostics
(
    const FrequencyDiagnostics& other
) : _name(other._name),
...
void FrequencyDiagnostics::Tick()
{
    _freq_status.tick();    
}
...
bool RosSensor::getUpdatedProfiles
(
...
)
{
    ...
    return !compare_profiles_lists(active_profiles, wanted_profiles);
}
...
rmw_qos_profile_t RosSensor::getQOS
(
...
rmw_qos_profile_t RosSensor::getInfoQOS
(
    const stream_index_pair& sip
) const
{
...
void RosSensor::setupErrorCallback()
{
    set_notifications_callback([&](const rs2::notification& n) {
        ...
        auto it = find_if(error_strings.begin(), error_strings.end(), [&n] (std::string err) { return (n.get_description().find(err) != std::string::npos); });
        if (error_strings.end() != it) {
...
void RosSensor::UpdateSequenceIdCallback()
{
    ...
    auto deleter_to_revert_hdr = std::unique_ptr<bool, std::function<void(bool*)>>(
        &is_hdr_enabled,
...
template<class T> 
void RosSensor::set_sensor_parameter_to_ros
(
    rs2_option option
)
{
...
bool RosSensor::profiles_equal
(
    const rs2::stream_profile& a, 
    const rs2::stream_profile& b
)
{
...
bool RosSensor::is_profiles_in_profiles
(
    const std::vector<stream_profile>& sub_profiles, 
    const std::vector<stream_profile>& all_profiles
)
{
...
bool RosSensor::compare_profiles_lists
(
    const std::vector<stream_profile>& active_profiles, 
    const std::vector<stream_profile>& wanted_profiles
)
{
