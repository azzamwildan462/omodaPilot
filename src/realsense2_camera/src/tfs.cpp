...
void BaseRealSenseNode::restartStaticTransformBroadcaster()
{
    ...
    _static_tf_broadcaster = std::make_shared<tf2_ros::StaticTransformBroadcaster>(
        _node, 
...
void BaseRealSenseNode::append_static_tf_msg
(
    const rclcpp::Time& t,
    const float3& trans,
...
void BaseRealSenseNode::erase_static_tf_msg
(
...
)
{
    ...
    it = std::find_if(_static_tf_msgs.begin(), _static_tf_msgs.end(), compare_tf_ids(frame_id, child_frame_id));
...
void BaseRealSenseNode::publishExtrinsicsTopic
(
...
tf2::Quaternion BaseRealSenseNode::rotationMatrixToQuaternion
(
...
void BaseRealSenseNode::calcAndAppendTransformMsgs
(
...
{
    if (!_publish_tf) {
        ROS_WARN("Since the param 'publish_tf' is set to 'false', the value set on the param 'tf_publish_rate' won't have any effect");
        return;
    }
    ...
    if (_tf_publish_rate > 0) {
        ...
        if (_tf_t && _tf_t->joinable()) {
            ...
            ROS_WARN("Currently not publishing dynamic camera transforms (/tf). To start publishing it, set the 'tf_publish_rate' param to > 0.0 Hz");
        }
    }
}
