...
void FramosRealSenseNodeFactory::init()
{
    try {
        ...
        _reconnect_timeout = declare_parameter("reconnect_timeout", 6.0); // _reconnect_timeout = declare_parameter("reconnect_timeout", rclcpp::ParameterValue(6.0)).get<rclcpp::PARAMETER_DOUBLE>();
        ...
        if (!_serial_no.empty() && _serial_no.front() == '_') {
...
void FramosRealSenseNodeFactory::changeDeviceCallback
(
...
void FramosRealSenseNodeFactory::getDevice
(
    ...
    if(!_device) {
        if (list.size() == 0) {
            ...
            for (size_t count = 0; count < list.size(); count++) {
                ...
                if (found_ip) { 
                    ip = dev.get_info(RS2_CAMERA_INFO_IP_ADDRESS);
                }
                ...
                if ((_ip_address.empty() || (ip == _ip_address && found_ip)) && 
                    (_serial_no.empty() || sn == _serial_no) && 
                    (_usb_port_id.empty() || port_id == _usb_port_id) 
                    && found_device_type
                ) {
                    _device = dev;
                    _serial_no = sn;
...
void FramosRealSenseNodeFactory::tryGetLogSeverity
(
...
std::string FramosRealSenseNodeFactory::parseUsbPort
(
    std::string line
)
{
    ...
    if (std::regex_match(line, base_match, self_regex)) {
        ...
        if (base_match[2].str().size() == 0) { //This is libuvc string. Remove counter is exists.
            ...
            if (std::regex_match(port_id, base_match, end_regex)) {
...
std::string FramosRealSenseNodeFactory::api_version_to_string
(
    int version
)
{
...
uint64_t FramosRealSenseNodeFactory::serialFromString
(
...
uint32_t FramosRealSenseNodeFactory::ipFromString
(
    const std::string& serial
)
{
...
bool FramosRealSenseNodeFactory::isNumber
(
