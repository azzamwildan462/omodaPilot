...
void Parameters::setRosParamValue
(
...
void Parameters::pushUpdateFunctions
(
...
)
{
    ...
    _param_functions[param_name] = (
        (func) ? 
            func :
            _param_functions[param_name] = [this](const rclcpp::Parameter&) { ROS_WARN_STREAM("Parameter can not be changed in runtime."); }
    );
...
)
{
    param = setParam<T>(
        param_name, 
        param, 
        [&param, func](const rclcpp::Parameter& parameter){ param = parameter.get_value<T>(); if (func) { func(parameter); } }, 
        descriptor
    );
}
...
template <class T>
T Parameters::readAndDeleteParam
(
    std::string param_name, 
    const T& initial_value
)
...
template <class T>
void Parameters::queueSetRosValue
(
    const std::string& param_name, 
    const T value
)
