...
rs2::frameset NamedFilter::Process
(
...
(
    std::function<void(const rclcpp::Parameter&)> enable_param_func
)
{
    ...
    if (use_texture) {
        ...
        texture_frame_itr = std::find_if(
            ...
            [&texture_source_id, &available_formats] (rs2::frame f) { 
                return (rs2_stream(f.get_profile().stream_type()) == texture_source_id) && (available_formats.find(f.get_profile().format()) != available_formats.end()); 
            }
        );
            ...
            if (valid_pixel || _ordered_pc) {
                ...
                *iter_z = vertex->z;
                ...
                if (valid_color_pixel) {
                    ...
                    auto reverse_memcpy = [](unsigned char* dst, const unsigned char* src, size_t n) {
...
void PointcloudFilter::setParameters()
{
    ...
    _pointcloud_qos = _params.getParameters()->setParam<std::string>(
        param_name, 
        DEFAULT_QOS, 
    ...
    NamedFilter::setParameters([this](const rclcpp::Parameter&) { setPublisher(); });
}



AlignDepthFilter::AlignDepthFilter
(
    std::shared_ptr<rs2::filter> filter,
