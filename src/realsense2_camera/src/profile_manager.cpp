...
) : _logger(logger), _params(parameters, _logger)
{
    // NOTHING
}
...
std::string ProfilesManager::profile_string
(
...
void ProfilesManager::registerSensorQOSParam
(
    std::string template_name, 
...
)
{
    ...
    for (auto& sip : unique_sips) {
        ...
        _params.getParameters()->setParam<std::string>(
            param_name, 
            value, 
            [this, param](const rclcpp::Parameter& parameter) {
                try {
...
void ProfilesManager::addWantedProfiles
(
    std::vector<rs2::stream_profile>& wanted_profiles
)
{    
    ...
    for (auto const & profile : _enabled_profiles) {
        ...
        if (*stream_enabled && !found_sips[sip]) {
            ROS_WARN_STREAM(
                "Couldn't open " << ros_stream_to_string(sip.first) << ":" << sip.second << " stream "
...
bool ProfilesManager::hasSIP
(
    const stream_index_pair& sip
) const
{
...
rmw_qos_profile_t ProfilesManager::getQOS
(
    const stream_index_pair& sip
) const
{
...
rmw_qos_profile_t ProfilesManager::getInfoQOS
(
    const stream_index_pair& sip
) const
{
...
template<class T>
void ProfilesManager::registerSensorUpdateParam
(
    std::string template_name, 
    std::set<stream_index_pair> unique_sips, 
    std::map<stream_index_pair, 
...
)
{
    ...
    for (auto& sip : unique_sips) {
        ...
        if (params.find(sip) == params.end()) {
            if (sip == INFRA0) {
                params[sip] = std::make_shared<T>(false); // Disabling Infra 0 stream by default
            }
            else {
        ...
        _params.getParameters()->setParam<T>(
            param_name, 
            *(params[sip]), 
            [param, update_sensor_func](const rclcpp::Parameter& parameter) {
...
VideoProfilesManager::VideoProfilesManager
(
    std::shared_ptr<Parameters> parameters,
...
int VideoProfilesManager::getHeight
(
    rs2_stream stream_type
) 
{ 
    return _height[stream_type];
}



int VideoProfilesManager::getWidth
(
    rs2_stream stream_type
) 
{ 
    return _width[stream_type];
}



int VideoProfilesManager::getFPS
(
    rs2_stream stream_type
) 
{ 
    return _fps[stream_type];
}



bool VideoProfilesManager::isWantedProfile
(
    const rs2::stream_profile& profile
)
{
...
void VideoProfilesManager::registerProfileParameters
(
    std::vector<stream_profile> all_profiles, 
    std::function<void()> update_sensor_func
)
{
...
bool VideoProfilesManager::isSameProfileValues
(
    const rs2::stream_profile& profile, 
    const int width, 
    const int height, 
...
)
{
    ...
    return (
        ...
        (RS2_FORMAT_ANY == format || video_profile.format() == format)
    );
}



rs2::stream_profile VideoProfilesManager::validateAndGetSuitableProfile
(
    rs2_stream stream_type, 
    rs2::stream_profile given_profile
)
{
...
void VideoProfilesManager::registerVideoSensorProfileFormat
(
    stream_index_pair sip
)
{
    if (sip == DEPTH) _formats[DEPTH] = RS2_FORMAT_Z16;
    else if (sip == INFRA0) _formats[INFRA0] = RS2_FORMAT_RGB8;
    else if (sip == INFRA1) _formats[INFRA1] = RS2_FORMAT_Y8;
    else if (sip == INFRA2) _formats[INFRA2] = RS2_FORMAT_Y8;
    else if (sip == COLOR) _formats[COLOR] = RS2_FORMAT_RGB8;
    else _formats[sip] = RS2_FORMAT_ANY;
}
...
(
    std::set<stream_index_pair> sips
)
{
    ...
    for (auto stream_type : available_streams) {
        ...
        _params.getParameters()->setParam<std::string>(
            param_name_str.str(), 
            crnt_profile_str.str(), 
            [this, stream_type](const rclcpp::Parameter& parameter) {
    ...
    for (auto sip : sips) {
        ...
        _params.getParameters()->setParam(
            param_name, 
            param_value, 
            [this, sip](const rclcpp::Parameter& parameter) {
                    ...
                    if (!found) {
                        ...
                        _params.getParameters()->queueSetRosValue(parameter.get_name(), (std::string)rs2_format_to_string(_formats[sip]));
                    }
                }, 
                crnt_descriptor
            );
...
std::string VideoProfilesManager::getProfileFormatsDescriptions
(
    stream_index_pair sip
)
{
...
bool MotionProfilesManager::isWantedProfile
(
    const rs2::stream_profile& profile
)
{
...
void MotionProfilesManager::registerProfileParameters
(
    std::vector<stream_profile> all_profiles, 
    std::function<void()> update_sensor_func
)
...
void MotionProfilesManager::registerFPSParams()
{
    ...
    for (auto& fps : _fps) {
        ...
        _params.getParameters()->setParam<int>(
            param_name, 
            *(fps.second), 
            [this, sip](const rclcpp::Parameter& parameter) {
...
bool MotionProfilesManager::isSameProfileValues
(
    const rs2::stream_profile& profile, 
    const rs2_stream stype, 
    const int fps
)
{
