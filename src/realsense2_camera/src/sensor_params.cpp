...
std::map<std::string, int> get_enum_method
(
    rs2::options sensor, 
    rs2_option option
)
...
void SensorParams::registerDynamicOptions
(
    rs2::options sensor,
...
std::shared_ptr<Parameters> SensorParams::getParameters()
{
    return _parameters;
}
...
double SensorParams::float_to_double
(
    float val, 
...
template<class T>
rcl_interfaces::msg::ParameterDescriptor SensorParams::get_parameter_descriptor
(
...
template<class T>
void SensorParams::set_parameter
(
    rs2::options sensor, 
    rs2_option option, 
    const std::string& module_name, 
...
)
{
    ...
    rcl_interfaces::msg::ParameterDescriptor parameter_descriptor = get_parameter_descriptor(
                                                                        option_name, 
                                                                        option_range, 
                                                                        option_value, 
    ...
    try {
        new_val = (
            _parameters->setParam<T>(
                option_name, 
                option_value, 
                [option, sensor](const rclcpp::Parameter& parameter) { param_set_option<T>(sensor, option, parameter); }, 
